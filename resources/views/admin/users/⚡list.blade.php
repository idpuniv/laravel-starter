        ...
        if ($this->statusFilter) {
            switch ($this->statusFilter) {
                ...
                case UserStatus::BANNED->value:
                    $query->whereHas('user', function ($q) {
                        $q->where('status', UserStatus::BANNED->value);
                    });
                    break;
            }
        }
        ...
        return response()->streamDownload(function () use ($data) {
            return new FastExcel($data)->export('php://output', function ($person) {
                return [
                    ...
                    'Statut' => $person->user ? $person->user->status->label() : 'Sans compte',
            ...
            @if ($statusFilter)
                <span class="badge text-dark border">
                    ...
                    @switch($statusFilter)
                        ...
                        @break
                        @case(UserStatus::INACTIVE->value)
                            {{ UserStatus::INACTIVE->label() }}
                        @break
                        @case(UserStatus::BANNED->value)
                            {{ UserStatus::BANNED->label() }}
                        @break
                    @endswitch
                    <button type="button" class="btn-close btn-close-sm ms-2" style="font-size: 0.6rem;"
                            ...
                            <div class="d-flex align-items-center gap-2">
                                @can(\App\Permissions\UserPermissions::VIEW)
                                    <a href="{{ route('admin.users.show', $person->id) }}"
                                        class="text-decoration-none text-body" title="Voir">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                @endcan
                                @can(\App\Permissions\UserPermissions::UPDATE)
                                    <a href="{{ route('admin.users.edit', $person->id) }}"
                                        class="text-decoration-none text-body" title="Modifier">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                @endcan
                                @can(\App\Permissions\UserPermissions::DELETE)
                                    <button type="button" class="btn btn-link text-danger p-0 m-0 text-decoration-none"
